...
class CustomDiffusionDataset(Dataset):
    ...
    def __init__(
        ...
        hflip=False,
        aug=True,
        use_various_class_prompts=False,
    ):
        self.size = size
        ...
        self.use_various_class_prompts = use_various_class_prompts
        ...
        self.image_transforms = transforms.Compose(
            [
                ...
                transforms.Lambda(lambda x: transforms.functional.crop(x, 0, 0, size, size)),
                transforms.ToTensor(),
    ...
    def preprocess(self, image, scale, resample):
        ...
        center_crop_transform = transforms.Compose(
            [
                transforms.Resize(scale, interpolation=transforms.InterpolationMode.BILINEAR),
                transforms.CenterCrop(scale),
            ]
        )
        image = center_crop_transform(image)  # resize and center crop
    ...
    def __getitem__(self, index):
        ...
        if self.with_prior_preservation:
            class_image_path, class_prompt = self.class_images_path[index % self.num_class_images]
            if self.use_various_class_prompts:
                class_prompt = class_image_path.name.split('--')[1][:-4]
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--use_various_class_prompts",
        action="store_true",
        help="If specified use SD3 generated images with various prompts when calculating prior preservation loss.",
    )
...
def main(args):
    ...
    train_dataset = CustomDiffusionDataset(
        ...
        hflip=args.hflip,
        aug=not args.noaug,
        use_various_class_prompts=args.use_various_class_prompts,
    )
