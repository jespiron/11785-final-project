...
class Attention(nn.Module):
    ...
    def get_attention_scores_without_softmax(self, query, key, attention_mask=None):
        dtype = query.dtype
        if self.upcast_attention:
            query = query.float()
            key = key.float()
        ...
        if self.upcast_softmax:
            attention_scores = attention_scores.float().to(dtype)
...
class AnalysisAttnProcessor3(nn.Module):
    r"""
    Processor for performing attention-related computations with extra learnable key and value matrices for the text
    encoder.
    For Analysis use, this processor computes the attention map of picked prompts
    Major difference w.r.t AnalysisAttnProcessor, AnalysisAttnProcessor2 is that this processor's analysis is based on
    the attention map, not the attention output.
    Q: "m1 c1 and m2 c2"
    K, V: list of picked prompts, e.g. "m1 c1"
    output: output of attention (same as AnalysisAttnProcessor, but use ordering on attention output value.)
    """
    ...
    def __call__(
        ...
        class_attn_score_dict=None,
        is_only_cross_concept_losses_on_prior=None,
        is_prior_minus_concept=None,
        placeholder_tokens=None,
        initializer_tokens=None,
        prompt_dict_list=None,
        attention_map_dict=None,
        init_attention_map_dict=None,
        instance_mask=None,
        block_count=None,
        val_timestep_count=None,
        val_image_count=None,
        timestep=None,
        adapter_attn_map=None,
        adapter_output=None,
        is_gaussian_blur=None,
        concept_mask_threshold=0.5,
        concept_mask_temperature=0.4,
        text_embs_list=None,
        prompt_list=None,
        value_idx_list=None,
        num_idx=0,
        ...
        is_inference = False if cross_concept_losses is not None else True

        #### added temporally for saving attn probs on WandB #####
        if block_count is not None:
            block_count[0] = block_count[0] + 1
        ##########################################################
        ...
        if is_cross_attn:
            ...
            # key_weight_detached = encoder_hidden_states @ self.to_k_custom_diffusion.weight.data.T
            # value_weight_detached = encoder_hidden_states @ self.to_v_custom_diffusion.weight.data.T
            # detach = torch.ones_like(key)
            # detach[1, :, :] = detach[1, :, :] * 0.0
            #
            # key = detach * key + (1 - detach) * key_weight_detached
            # value = detach * value + (1 - detach) * value_weight_detached
        else:
            key = attn.to_k(encoder_hidden_states)
            value = attn.to_v(encoder_hidden_states)
        ...
        batch_heads, spatial_dim, _ = attention_probs.shape

        ############### save attention score on WandB ###############
        # since attn_probs is averaged in heads_dimension, *man's attn_probs might not look (1-*woman's attn_probs)
        if is_cross_attn and (timestep[0] % 250 < 20 or timestep[0] == 20):
            states_list = []
            for text_embs, value_idx in zip(text_embs_list, value_idx_list):
                text_embs_key = self.to_k_custom_diffusion(text_embs)
                text_embs_key = attn.head_to_batch_dim(text_embs_key)

                text_embs_value = self.to_v_custom_diffusion(text_embs)
                text_embs_value = attn.head_to_batch_dim(text_embs_value)

                query_temp = attn.batch_to_head_dim(query)[[num_idx, num_idx+batch_size//2],:,:]
                query_temp = attn.head_to_batch_dim(query_temp)
                text_embs_attention_probs = attn.get_attention_scores_without_softmax(query_temp, text_embs_key)[:, :, value_idx]
                text_embs_state = attn.batch_to_head_dim(text_embs_attention_probs)
                # text_embs_state = torch.bmm(text_embs_attention_probs, text_embs_value)
                # text_embs_state = attn.batch_to_head_dim(text_embs_state)

                # text_embs_state = attn.to_out[0](text_embs_state)
                # text_embs_state = attn.to_out[1](text_embs_state)
                states_list.append(text_embs_state)

            # result_names = ["new1", "man", "new2", "woman", "new1_man", "new1_woman", "new2_man", "new2_woman", "original"]

            if f'timestep_{timestep[0]}' not in result_dict.keys():
                result_dict[f'timestep_{timestep[0]}'] = {
                    'original': {}
                }
                result_dict[f'timestep_{timestep[0]}']['shape16'] = {}
            for idx, name in enumerate(prompt_list):
                # result_dict[f'timestep_{timestep[0]}']['original'][f'{name}'] = states_list[idx]
                if text_embs_attention_probs.shape[1] == 256:
                    result_dict[f'timestep_{timestep[0]}']['shape16'][f'{name}'] = states_list[idx]
        ...
        if block_count is not None and \
                block_count[0] == 32:
            block_count[0] = 0
            if val_timestep_count is not None:
                val_timestep_count[0] += 1
                if val_timestep_count[0] == 50:
                    val_timestep_count[0] = 0
                    if val_image_count is not None:
                        val_image_count[0] += 1
