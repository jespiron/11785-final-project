...
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), "../customedit"))
from custom_edit import main
...
class GaussCtrlPipeline(VanillaPipeline):
    ...
    def save_image(self, _array, filepath):
        # Ensure array is in uint8 format for saving as an image
        _array = _array.astype(np.uint8)

        # Convert NumPy array to PIL Image and save
        image = Image.fromarray(_array)
        image.save(filepath)
    ...
    def edit_images(self):
        ...
        editing_prompt = self.edit_prompt
        output_folder = '../temp_output4'
        original_words = ('horse', )
        edit_words = ('<new1>', )
        ...
        for idx in range(0, len(self.datamanager.train_data), self.chunk_size): 
            ...
            # imageg path or iamge array should be a numpy array
            edited_images = []
            for _idx in range(len(indices)):
                # print(unedited_images[_idx].shape)
                # print(unedited_images[_idx].max())
                # print(unedited_images[_idx].min())
                input_img = np.array(unedited_images[_idx] * 255).astype(np.uint8)
                self.save_image(input_img, os.path.join(output_folder, f'input_img_{indices[_idx]}.jpg'))
                if mask_images != []:
                    mask = mask_images[_idx]
                    self.save_image((mask * 255).astype(np.uint8), os.path.join(output_folder, f'mask_{indices[_idx]}.jpg'))
                    mask_input_img = (input_img * mask[None].reshape(512, 512, 1)).astype(np.uint8)
                    self.save_image(mask_input_img, os.path.join(output_folder, f'input_img_mask_{indices[_idx]}.jpg'))
                    # input_img = mask_input_img

                # print(input_img.shape)
                # print(self.reverse_prompt)
                edited_image = main(
                    image_path_or_image_array=input_img,
                    prompt=self.reverse_prompt,
                ...
                edited_images.append(torch.from_numpy(edited_image[1]).permute(2, 0, 1).unsqueeze(0).float() / 255)
            ...
            chunk_edited = torch.cat(edited_images)
            # print(chunk_edited.shape)  # (3, 3, 512, 512)
